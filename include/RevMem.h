...
public:
  ...
  void RmtLR( unsigned Hart, uint64_t Nmspace, uint64_t Addr, size_t Size, void* Target, const RmtMemReq& Req, RevFlag Flags ) {
    rmtCtrl->sendRmtReadLockRqst( Hart, Nmspace, Addr, Size, Target, Req, Flags );
  }

  ///  RevMem: template remote STORE CONDITIONAL memory interface
  void RmtSC(
    unsigned Hart, uint64_t Nmspace, uint64_t Addr, size_t Size, void* Data, void* Target, const RmtMemReq& Req, RevFlag Flags
  ) {
    uint8_t* DataMem = (uint8_t*) ( Data );
    rmtCtrl->sendRmtWriteUnLockRqst( Hart, Nmspace, Addr, Size, Req, Flags, DataMem, Target );
  }
