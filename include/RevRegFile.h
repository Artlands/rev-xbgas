...
enum class RevReg : uint16_t {
  ...
  e8   =  8, e9  =  9, e10  = 10, e11  = 11, e12 = 12, e13 = 13, e14  = 14, e15  = 15,
  e16  = 16, e17 = 17, e18  = 18, e19  = 19, e20 = 20, e21 = 21, e22  = 22, e23  = 23,
  e24  = 24, e25 = 25, e26  = 26, e27  = 27, e28 = 28, e29 = 29, e30  = 30, e31  = 31,
};
// clang-format on
...
public:
  ...
  void SetRmtLSQueue( std::shared_ptr<std::unordered_multimap<uint64_t, RmtMemReq>> lsq ) { RmtLSQueue = std::move( lsq ); }
  ...
  /// GetE: Get the Extended E register of xBGAS
  template<typename T, typename U>
  T GetE( U rs ) const {
    T res;
    if( IsRV64() ) {
  ...
  // xBGAS load-store functions
  template<typename T>
  friend bool eload( const RevFeature* F, RevRegFile* R, RevMem* M, const RevInst& Inst );

  template<typename T>
  friend bool estore( const RevFeature* F, RevRegFile* R, RevMem* M, const RevInst& Inst );

  template<typename T>
  friend bool erload( const RevFeature* F, RevRegFile* R, RevMem* M, const RevInst& Inst );

  template<typename T>
  friend bool erstore( const RevFeature* F, RevRegFile* R, RevMem* M, const RevInst& Inst );

  template<typename T>
  friend bool ebload( const RevFeature* F, RevRegFile* R, RevMem* M, const RevInst& Inst );

  template<typename T>
  friend bool ebstore( const RevFeature* F, RevRegFile* R, RevMem* M, const RevInst& Inst );
