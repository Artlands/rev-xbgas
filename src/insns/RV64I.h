...
namespace SST{
  namespace RevCPU{
    class RV64I : public RevExt {
      ...
      //   int32_t tmp = Inst.imm & 0b111111;
      //   R->RV64[Inst.rd] = (int64_t)(R->RV64[Inst.rs1] << tmp);
      //   R->RV64_PC += Inst.instSize;
      //   return true;
      // }
      ...
      static bool addiw(RevFeature *F, RevRegFile *R,RevMem *M,RevXbgas *Xbgas,RevInst Inst) {
        int32_t tmp = (int32_t)(td_u32(Inst.imm,12));
        R->RV64[Inst.rd] = dt_u64((int32_t)(td_u64(R->RV64[Inst.rs1],64)) + tmp,64);
      ...
      std::vector<RevInstEntry > RV64ITable = {
        ...
        {RevInstEntryBuilder<RevInstDefaults>().SetMnemonic("slliw %rd, %rs1, $imm" ).SetCost(1).SetOpcode( 0b0011011 ).SetFunct3( 0b001 ).SetFunct7(0b0000000).SetrdClass(RegGPR    ).Setrs1Class(RegGPR).Setrs2Class(RegUNKNOWN).Setimm12(0b0).Setimm(FUnk).SetFormat(RVTypeI).SetImplFunc( &slliw ).InstEntry},
        {RevInstEntryBuilder<RevInstDefaults>().SetMnemonic("srliw %rd, %rs1, $imm" ).SetCost(1).SetOpcode( 0b0011011 ).SetFunct3( 0b101 ).SetFunct7(0b0000000).SetrdClass(RegGPR    ).Setrs1Class(RegGPR).Setrs2Class(RegUNKNOWN).Setimm12(0b0).Setimm(FUnk).SetFormat(RVTypeI).SetImplFunc( &srliw ).InstEntry},
        {RevInstEntryBuilder<RevInstDefaults>().SetMnemonic("sraiw %rd, %rs1, $imm" ).SetCost(1).SetOpcode( 0b0011011 ).SetFunct3( 0b101 ).SetFunct7(0b0100000).SetrdClass(RegGPR    ).Setrs1Class(RegGPR).Setrs2Class(RegUNKNOWN).Setimm12(0b0).Setimm(FUnk).SetFormat(RVTypeI).SetImplFunc( &sraiw ).InstEntry},
