...
sst.setStatisticLoadLevel(4)  # Enable detailed statistics (0 to 10, where 4 is moderate detail)

sst.setStatisticOutput("sst.statOutputCSV")  # Output statistics in CSV format

# Enabling statistics for memory controllers
memctrl.enableAllStatistics({
    "type": "sst.AccumulatorStatistic",
    "rate": "100ns"
})

# Enabling statistics for NIC (network interface)
rmt_nic_iface.enableAllStatistics({
    "type": "sst.AccumulatorStatistic",
    "rate": "100ns"
})

# Enabling statistics for the CPU (RevCPU)
xbgas_cpu.enableAllStatistics({
    "type": "sst.AccumulatorStatistic",
    "rate": "100ns"
})

# Specify which statistics to gather for each component
cpu_stats = ["instruction_count", "cycle_count", "loads_issued", "stores_issued", "instructions_completed"]
mem_stats = ["bytes_read", "bytes_written", "bandwidth"]
nic_stats = ["packets_sent", "packets_received", "packet_bandwidth"]

# Enable CPU statistics
for stat in cpu_stats:
    xbgas_cpu.enableStatistic(stat, {"rate": "100ns"})

# Enable memory controller statistics
for stat in mem_stats:
    memctrl.enableStatistic(stat, {"rate": "100ns"})

# Enable NIC statistics for bandwidth
for stat in nic_stats:
    rmt_nic_iface.enableStatistic(stat, {"rate": "100ns"})

# Optional: Output statistics to CSV for easier analysis
sst.setStatisticOutputOptions({
    "filepath": "sst_bandwidth_stats.csv",
    "separator": ", "
})
