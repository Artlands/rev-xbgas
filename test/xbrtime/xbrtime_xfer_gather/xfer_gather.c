...
int main( int argc, char **argv ){
  ...
  int src_array[2] = {my_pe + 1, my_pe * 2};
  ...
  if (my_pe == root_pe) {
    revprintf("Post-Gather PE %d dest_array = %d %d | %d %d | %d %d | %d %d | %d %d\n", 
             my_pe, dest_array[0], dest_array[1], dest_array[2], dest_array[3], dest_array[4], 
                    dest_array[5], dest_array[6], dest_array[7], dest_array[8], dest_array[9]);
  }
