...
static void vprintfmt( void ( *putch )( int, void** ), void** putdat, const char* fmt, va_list ap ) {
  ...
  reswitch:
    ...
    case 'f': {
      double fnum = va_arg( ap, double );  // Retrieve the floating-point number
      if( fnum < 0 ) {
        putch( '-', putdat );
        fnum = -fnum;
      }
      ...
      unsigned long long ipart = (unsigned long long) floor( fnum );
      double             fpart = fnum - ipart;

      // Print integer part
      printnum( putch, putdat, ipart, 10, width, padc );
      ...
      if( precision > 0 ) {
        ...
        unsigned long long frac_part = (unsigned long long) ( fpart + 0.5 );  // Round to nearest digit
        printnum( putch, putdat, frac_part, 10, precision, '0' );
      }
      break;
    }
