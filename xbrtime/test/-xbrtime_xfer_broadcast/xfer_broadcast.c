...
int main( int argc, char **argv ){
  int i, numpes, my_pe, root_pe, nelems, stride, total_nelems, *b_val;
  root_pe = 0;
  ...
  b_val = (int*)xbrtime_malloc(total_nelems * sizeof(int));

  for (i = 0; i < total_nelems; i++){
    b_val[i] = i + my_pe;
  }
  ...
  if (my_pe == root_pe) {
    revprintf("PE %d BROADCASTs %d elements with stride of %d to other PEs\n", root_pe, nelems, stride);
  }
